...
import { askSatQueryLlm } from "./llm";
...
export function SatQueryProvider({ children }: { children: ReactNode }) {
  ...
  const localRun = useCallback((q: string, m: Mission, id: number) => {
    ...
    const imageUrl = m.assets.find((a) => a.role === "primary")?.src ?? m.assets[0]?.src;
    const llmPromise = askSatQueryLlm({
      query: q,
      task: next.task,
      imageUrl,
      context: [next.title, ...next.observations, ...next.metrics.map((metric) => `${metric.label}: ${metric.value}`)].join(
        "\n"
      ),
    });
          ...
          try {
            ...
            if (llm?.answer) {
              used = {
                ...
                answer: llm.answer,
                observations: [
                  `Answer generated by ${llm.model}; overlays below come from the evidence stack.`,
                  ...next.observations,
                ],
              };
            }
          } catch {
            /* keep the scripted answer */
          }
          if (runId.current !== id) return;
          setSteps((prev) => prev.map((s) => ({ ...s, status: "done" })));
          setResult(used);
          if (used.compareDefault) setCompare(used.compareDefault);
          setRunning(false);
          setThread((t) => [
            ...t,
            { id: `a-${id}`, role: "instrument", text: used.answer, result: used },
          ]);
          window.setTimeout(() => {
