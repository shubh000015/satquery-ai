...
from app.tools.endpoint_client import ask_grounding, ask_vqa_with_bands, decode_rle
...
def _endpoint_answer(tool: Tool, ctx: ToolContext, endpoint: str, task: str) -> ToolOutput:
    ...
    reply = ask_vqa_with_bands(
        endpoint,
        ctx.primary_scene,
        ctx.primary_asset.modality,
        ctx.query,
        task=task,
    )
    ...
    for finding in reply.findings:
        if finding and finding not in output.observations:
            output.observations.append(finding)
